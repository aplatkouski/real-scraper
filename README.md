
# [Core Python Language Tutorials](https://realpython.com/tutorials/python/)

 - [ ] [Cool New Features in Python 3.7](https://realpython.com/python37-new-features/)
`python`

 - [ ] [8 World-Class Software Companies That Use Python](https://realpython.com/world-class-companies-using-python/)
`python`

 - [ ] [Python KeyError Exceptions and How to Handle Them](https://realpython.com/python-keyerror/)
*Apr 24, 2019* | `python` `basics`

 - [ ] Course: [Python KeyError Exceptions and How to Handle Them](https://realpython.com/courses/python-keyerror/)
*Nov 26, 2019* | `python` `basics`



# [Python Basics](https://realpython.com/tutorials/basics/)

 - [ ] [Introduction to Python 3](https://realpython.com/python-introduction/)
`python` `basics`

   - [ ] [Python 3 Installation & Setup Guide](https://realpython.com/installing-python/)
`best-practices` `python` `basics`

   - [ ] [Interacting with Python](https://realpython.com/interacting-with-python/)
`python` `basics`

   - [ ] [Basic Data Types in Python](https://realpython.com/python-data-types/)
`python` `basics`

   - [ ] [Variables in Python](https://realpython.com/python-variables/)
`python` `basics`

   - [ ] [Operators and Expressions in Python](https://realpython.com/python-operators-expressions/)
`python` `basics`

   - [ ] [Strings and Character Data in Python](https://realpython.com/python-strings/)
`python` `basics`

   - [ ] [Lists and Tuples in Python](https://realpython.com/python-lists-tuples/)
`python` `basics`

   - [ ] [Dictionaries in Python](https://realpython.com/python-dicts/)
`python` `basics`

   - [ ] [Sets in Python](https://realpython.com/python-sets/)
`python` `basics`

   - [ ] [Python Program Lexical Structure](https://realpython.com/python-program-structure/)
`python` `basics`

   - [ ] [Conditional Statements in Python](https://realpython.com/python-conditional-statements/)
`python` `basics`

   - [ ] [Python "while" Loops (Indefinite Iteration)](https://realpython.com/python-while-loop/)
`python` `basics`

   - [ ] [Python "for" Loops (Definite Iteration)](https://realpython.com/python-for-loop/)
`python` `basics`

   - [ ] [Basic Input, Output, and String Formatting in Python](https://realpython.com/python-input-output/)
*Jun 24, 2019* | `python` `basics`

 - [ ] [Learn Python the Methodical Way](https://realpython.com/learn-python-the-methodical-way/)
`python` `basics`

 - [ ] Course: [Python Modules and Packages: An Introduction](https://realpython.com/courses/python-modules-packages/)
*Jan 28, 2020* | `python` `basics`

 - [ ] Course: [Playing and Recording Sound in Python](https://realpython.com/courses/playing-and-recording-sound-python/)
*Feb 11, 2020* | `python` `basics`

 - [ ] [How to Use the Python or Operator](https://realpython.com/python-or-operator/)
*Jul 03, 2019* | `python` `basics`

 - [ ] [How to Run Your Python Scripts](https://realpython.com/run-python-scripts/)
`python` `basics`

 - [ ] [11 Beginner Tips for Learning Python Programming](https://realpython.com/python-beginner-tips/)
`python` `basics`

 - [ ] Course: [Python Imports 101](https://realpython.com/courses/python-imports-101/)
`python` `basics`

 - [ ] [Working With Files in Python](https://realpython.com/working-with-files-in-python/)
`python` `basics`

 - [ ] Course: [Dictionaries in Python](https://realpython.com/courses/dictionaries-python/)
*Jul 30, 2019* | `python` `basics`

 - [ ] [How to Use any() in Python](https://realpython.com/any-python/)
*Mar 30, 2020* | `python` `basics`

 - [ ] [What Can I Do With Python?](https://realpython.com/what-can-i-do-with-python/)
`python` `basics`

 - [ ] [How to Use sorted() and sort() in Python](https://realpython.com/python-sort/)
*May 06, 2019* | `python` `basics`

 - [ ] Course: [Sorting Data With Python](https://realpython.com/courses/python-sorting-data/)
*Dec 31, 2019* | `python` `basics`

 - [ ] [Using the Python zip() Function for Parallel Iteration](https://realpython.com/python-zip-function/)
*Oct 02, 2019* | `python` `basics`

 - [ ] Course: [Strings and Character Data in Python](https://realpython.com/courses/python-strings/)
*Oct 01, 2019* | `python` `basics`

 - [ ] Course: [Python Exceptions 101](https://realpython.com/courses/python-exceptions-101/)
`python` `basics`

 - [ ] [Python Exceptions: An Introduction](https://realpython.com/python-exceptions/)
`python` `basics`

 - [ ] [Splitting, Concatenating, and Joining Strings in Python](https://realpython.com/python-string-split-concatenate-join/)
`python` `basics`

 - [ ] [A Guide to the Newer Python String Format Techniques](https://realpython.com/python-formatted-output/)
*Feb 17, 2020* | `python` `basics`

 - [ ] Course: [Sets in Python](https://realpython.com/courses/sets-python/)
*Feb 04, 2020* | `python` `basics`

 - [ ] [Playing and Recording Sound in Python](https://realpython.com/playing-and-recording-sound-python/)
*May 13, 2019* | `python` `basics`

 - [ ] [Understanding the Python Traceback](https://realpython.com/python-traceback/)
*Jul 29, 2019* | `python` `basics`

 - [ ] [Python Modules and Packages – An Introduction](https://realpython.com/python-modules-packages/)
`python` `basics`

 - [ ] Course: [The Python range() Function](https://realpython.com/courses/python-range-function/)
*Oct 15, 2019* | `python` `basics`

 - [ ] Course: [Splitting, Concatenating, and Joining Strings in Python](https://realpython.com/courses/splitting-concatenating-and-joining-strings-python/)
`python` `basics`

 - [ ] [How to Implement a Python Stack](https://realpython.com/how-to-implement-python-stack/)
*Jun 05, 2019* | `python` `basics`

 - [ ] [Python 3's f-Strings: An Improved String Formatting Syntax (Guide)](https://realpython.com/python-f-strings/)
`python` `basics`

 - [ ] Course: [Python 3's f-Strings: An Improved String Formatting Syntax](https://realpython.com/courses/python-3-f-strings-improved-string-formatting-syntax/)
`python` `basics`

 - [ ] Course: [11 Beginner Tips for Learning Python](https://realpython.com/courses/python-beginner-tips/)
*Aug 06, 2019* | `python` `basics`

 - [ ] [Invalid Syntax in Python: Common Reasons for SyntaxError](https://realpython.com/invalid-syntax-python/)
*Nov 20, 2019* | `python` `basics`

 - [ ] [Python GUI Programming With Tkinter](https://realpython.com/python-gui-tkinter/)
*Jan 22, 2020* | `python` `basics`

 - [ ] [The Python Square Root Function](https://realpython.com/python-square-root-function/)
*Jul 08, 2019* | `basics`

 - [ ] [Getting Started With Python IDLE](https://realpython.com/python-idle/)
*Nov 13, 2019* | `python` `basics`

 - [ ] Course: [For Loops in Python (Definite Iteration)](https://realpython.com/courses/python-for-loop/)
`python` `basics`

 - [ ] Course: [How to Implement a Python Stack](https://realpython.com/courses/python-stack/)
*Mar 03, 2020* | `python` `basics`

 - [ ] [The Python range() Function (Guide)](https://realpython.com/python-range/)
`python` `basics`

 - [ ] Course: [Installing Python on Windows, macOS, and Linux](https://realpython.com/courses/installing-python-windows-macos-linux/)
*May 16, 2019* | `python` `basics`

 - [ ] [Null in Python: Understanding Python's NoneType Object](https://realpython.com/null-in-python/)
*Feb 19, 2020* | `python` `basics`

 - [ ] Course: [Lists and Tuples in Python](https://realpython.com/courses/lists-tuples-python/)
*Sep 03, 2019* | `python` `basics`

 - [ ] [Your Guide to the Python print() Function](https://realpython.com/python-print/)
*Aug 12, 2019* | `python` `basics`

 - [ ] [First Steps With Python](https://realpython.com/python-first-steps/)
`python` `basics`

 - [ ] [Defining Your Own Python Function](https://realpython.com/defining-your-own-python-function/)
*Mar 09, 2020* | `python` `basics`

 - [ ] [Python Programming Challenge – First to Five](https://realpython.com/python-programming-contest-first-to-five/)
`python` `basics`

 - [ ] Course: [Running Python Scripts](https://realpython.com/courses/running-python-scripts/)
`python` `basics`

 - [ ] Course: [Variables in Python](https://realpython.com/courses/variables-python/)
*Dec 10, 2019* | `python` `basics`

 - [ ] [Using the Python defaultdict Type for Handling Missing Keys](https://realpython.com/python-defaultdict/)
*Mar 11, 2020* | `python` `basics`

 - [ ] [The Beginner's Guide to Python Turtle](https://realpython.com/beginners-guide-python-turtle/)
*Feb 26, 2020* | `python` `basics`

 - [ ] [How to Convert a Python String to int](https://realpython.com/convert-python-string-to-int/)
*Sep 18, 2019* | `python` `basics`

 - [ ] Course: [Mastering While Loops](https://realpython.com/courses/mastering-while-loops/)
`python` `basics`

 - [ ] Course: [Conditional Statements in Python (if/elif/else)](https://realpython.com/courses/python-conditional-statements/)
*Apr 25, 2019* | `python` `basics`

 - [ ] Course: [Introduction to Python Exceptions](https://realpython.com/courses/introduction-python-exceptions/)
`python` `basics`

 - [ ] Course: [Basic Data Types in Python](https://realpython.com/courses/python-data-types/)
*Jan 21, 2020* | `python` `basics`

 - [ ] [When to Use a List Comprehension in Python](https://realpython.com/list-comprehension-python/)
*Nov 06, 2019* | `python` `basics`



# [Intermediate Python Tutorials](https://realpython.com/tutorials/intermediate/)

 - [ ] [The Python pickle Module: How to Persist Objects in Python](https://realpython.com/python-pickle-module/)
`python` `intermediate`

 - [ ] [Primer on Python Decorators](https://realpython.com/primer-on-python-decorators/)
`intermediate` `python`

 - [ ] Course: [Python Decorators 101](https://realpython.com/courses/python-decorators-101/)
`intermediate` `python`

 - [ ] [Absolute vs Relative Imports in Python](https://realpython.com/absolute-vs-relative-python-imports/)
`best-practices` `intermediate` `python`

 - [ ] Course: [Absolute vs Relative Imports in Python](https://realpython.com/courses/absolute-vs-relative-imports-python/)
*Sep 10, 2019* | `best-practices` `intermediate` `python`

 - [ ] [Object-Oriented Programming (OOP) in Python 3](https://realpython.com/python3-object-oriented-programming/)
`intermediate` `python`

 - [ ] Course: [Intro to Object-Oriented Programming (OOP) in Python](https://realpython.com/courses/intro-object-oriented-programming-oop-python/)
`intermediate` `python`

 - [ ] [Inheritance and Composition: A Python OOP Guide](https://realpython.com/inheritance-composition-python/)
*Aug 07, 2019* | `best-practices` `intermediate` `python`

 - [ ] [Object-Oriented Programming in Python vs Java](https://realpython.com/oop-in-python-vs-java/)
*Jun 03, 2019* | `intermediate` `python`

 - [ ] Course: [Pythonic OOP String Conversion: __repr__ vs __str__](https://realpython.com/courses/pythonic-oop-string-conversion-__repr__-vs-__str__/)
`intermediate` `python`

 - [ ] Course: [OOP Method Types in Python: @classmethod vs @staticmethod vs Instance Methods](https://realpython.com/courses/python-method-types/)
*Jun 11, 2019* | `intermediate` `python`

 - [ ] Course: [@staticmethod vs @classmethod in Python](https://realpython.com/courses/staticmethod-vs-classmethod-python/)
`intermediate` `python`

 - [ ] [Supercharge Your Classes With Python super()](https://realpython.com/python-super/)
`best-practices` `intermediate` `python`

 - [ ] Course: [Supercharge Your Classes With Python super()](https://realpython.com/courses/python-super/)
*Jan 14, 2020* | `best-practices` `intermediate` `python`

 - [ ] [Python's Instance, Class, and Static Methods Demystified](https://realpython.com/instance-class-and-static-methods-demystified/)
`intermediate` `python`

 - [ ] [Operator and Function Overloading in Custom Python Classes](https://realpython.com/operator-function-overloading/)
`intermediate` `python`

 - [ ] [The Ultimate Guide to Data Classes in Python 3.7](https://realpython.com/python-data-classes/)
`intermediate` `python`

 - [ ] [Python '!=' Is Not 'is not': Comparing Objects in Python](https://realpython.com/python-is-identity-vs-equality/)
*Jan 29, 2020* | `best-practices` `intermediate` `python`

 - [ ] Course: [Comparing Python Objects the Right Way: "is" vs "=="](https://realpython.com/courses/python-is-identity-vs-equality/)
*Mar 31, 2020* | `best-practices` `intermediate` `python`

 - [ ] [Defining Main Functions in Python](https://realpython.com/python-main-function/)
*May 01, 2019* | `best-practices` `intermediate`

 - [ ] Course: [Defining Main Functions in Python](https://realpython.com/courses/python-main-function/)
*Mar 10, 2020* | `best-practices` `intermediate`

 - [ ] [How to Build Command Line Interfaces in Python With argparse](https://realpython.com/command-line-interfaces-python-argparse/)
*Jun 12, 2019* | `best-practices` `intermediate`

 - [ ] [Comparing Python Command-Line Parsing Libraries – Argparse, Docopt, and Click](https://realpython.com/comparing-python-command-line-parsing-libraries-argparse-docopt-click/)
`tools` `intermediate`

 - [ ] [Python Command Line Arguments](https://realpython.com/python-command-line-arguments/)
*Feb 05, 2020* | `best-practices` `tools` `intermediate`

 - [ ] [Python args and kwargs: Demystified](https://realpython.com/python-kwargs-and-args/)
*Sep 04, 2019* | `intermediate` `python`

 - [ ] Course: [Python args and kwargs: Demystified](https://realpython.com/courses/python-kwargs-and-args/)
*Jan 07, 2020* | `intermediate` `python`

 - [ ] [How to Round Numbers in Python](https://realpython.com/python-rounding/)
`best-practices` `intermediate` `python`

 - [ ] [The Factory Method Pattern and Its Implementation in Python](https://realpython.com/factory-method-python/)
`best-practices` `intermediate`

 - [ ] [The Most Diabolical Python Antipattern](https://realpython.com/the-most-diabolical-python-antipattern/)
`best-practices` `intermediate` `python`

 - [ ] [Memory Management in Python](https://realpython.com/python-memory-management/)
`intermediate` `python`

 - [ ] [An Intro to Threading in Python](https://realpython.com/intro-to-python-threading/)
`best-practices` `intermediate`

 - [ ] Course: [Threading in Python](https://realpython.com/courses/threading-python/)
*Nov 19, 2019* | `best-practices` `intermediate`

 - [ ] [Getting Started With Async Features in Python](https://realpython.com/python-async-features/)
*Sep 23, 2019* | `intermediate` `python`

 - [ ] [Async IO in Python: A Complete Walkthrough](https://realpython.com/async-io-python/)
`intermediate` `python`

 - [ ] [How to Make an Instagram Bot With Python and InstaPy](https://realpython.com/instagram-bot-python-instapy/)
*Apr 06, 2020* | `intermediate` `python`

 - [ ] [Python vs C++: Selecting the Right Tool for the Job](https://realpython.com/python-vs-cpp/)
*Sep 11, 2019* | `intermediate` `python`

 - [ ] [Pointers in Python: What's the Point?](https://realpython.com/pointers-in-python/)
*May 29, 2019* | `intermediate` `python`

 - [ ] [Python sleep(): How to Add Time Delays to Your Code](https://realpython.com/python-sleep/)
*Oct 16, 2019* | `intermediate` `python`

 - [ ] [Working With JSON Data in Python](https://realpython.com/python-json/)
`intermediate` `python`

 - [ ] Course: [Working With JSON Data in Python](https://realpython.com/courses/working-json-data-python/)
`intermediate` `python`

 - [ ] Course: [Immutability in Python](https://realpython.com/courses/immutability-python/)
*Apr 18, 2019* | `intermediate` `python`

 - [ ] [Python, Ruby, and Golang: A Command-Line Application Comparison](https://realpython.com/python-ruby-and-golang-a-command-line-application-comparison/)
`intermediate` `python`

 - [ ] [Python Inner Functions—What Are They Good For?](https://realpython.com/inner-functions-what-are-they-good-for/)
`intermediate` `python`

 - [ ] [Python and PyQt: Building a GUI Desktop Calculator](https://realpython.com/python-pyqt-gui-calculator/)
*Oct 23, 2019* | `intermediate`

 - [ ] [How to Build a Python GUI Application With wxPython](https://realpython.com/python-gui-with-wxpython/)
`intermediate`

 - [ ] [How to Use Generators and yield in Python](https://realpython.com/introduction-to-python-generators/)
*Sep 25, 2019* | `intermediate` `python`

 - [ ] [How to Iterate Through a Dictionary in Python](https://realpython.com/iterate-through-dictionary-python/)
*May 27, 2019* | `intermediate` `python`

 - [ ] [Cool New Features in Python 3.8](https://realpython.com/python38-new-features/)
*Oct 14, 2019* | `intermediate` `python`

 - [ ] Course: [Reading and Writing Files in Python](https://realpython.com/courses/reading-and-writing-files-python/)
*Jul 09, 2019* | `intermediate` `python`

 - [ ] [Python 3's pathlib Module: Taming the File System](https://realpython.com/python-pathlib/)
`intermediate` `python`

 - [ ] [Python Descriptors: An Introduction](https://realpython.com/python-descriptors/)
*Nov 27, 2019* | `intermediate` `python`

 - [ ] [Python Timer Functions: Three Ways to Monitor Your Code](https://realpython.com/python-timer/)
*Dec 30, 2019* | `intermediate` `python`

 - [ ] [How to Do a Binary Search in Python](https://realpython.com/binary-search-python/)
*Mar 16, 2020* | `intermediate` `python`

 - [ ] [Thinking Recursively in Python](https://realpython.com/python-thinking-recursively/)
`intermediate` `python`

 - [ ] [Arduino With Python: How to Get Started](https://realpython.com/arduino-python/)
*Oct 21, 2019* | `intermediate` `python`

 - [ ] Course: [Thinking Recursively in Python](https://realpython.com/courses/thinking-recursively-python/)
*Nov 12, 2019* | `intermediate` `python`

 - [ ] Course: [Python Context Managers and the "with" Statement](https://realpython.com/courses/python-context-managers-and-with-statement/)
*May 07, 2019* | `intermediate` `python`

 - [ ] Course: [How to Work With a PDF in Python](https://realpython.com/courses/pdf-python/)
*Feb 25, 2020* | `intermediate`

 - [ ] [Linked Lists in Python: An Introduction](https://realpython.com/linked-lists-python/)
*Apr 01, 2020* | `intermediate` `python`

 - [ ] [Python Scope & the LEGB Rule: Resolving Names in Your Code](https://realpython.com/python-scope-legb-rule/)
*Mar 18, 2020* | `intermediate` `python`

 - [ ] [Sorting Algorithms in Python](https://realpython.com/sorting-algorithms-python/)
`intermediate` `python`

 - [ ] [The Python math Module: Everything You Need to Know](https://realpython.com/python-math-module/)
*Mar 23, 2020* | `intermediate` `python`

 - [ ] Course: [Arduino With Python: How to Get Started](https://realpython.com/courses/arduino-python/)
*Apr 07, 2020* | `intermediate` `python`

 - [ ] [A Guide to Excel Spreadsheets in Python With openpyxl](https://realpython.com/openpyxl-excel-spreadsheets-python/)
*Aug 26, 2019* | `intermediate`

 - [ ] Course: [Cool New Features in Python 3.8](https://realpython.com/courses/cool-new-features-python-38/)
*Nov 05, 2019* | `intermediate` `python`

 - [ ] [PyGame: A Primer on Game Programming in Python](https://realpython.com/pygame-a-primer/)
*Sep 16, 2019* | `intermediate`

 - [ ] [Reading and Writing Files in Python (Guide)](https://realpython.com/read-write-files-python/)
`intermediate` `python`

 - [ ] [The Best Python Books](https://realpython.com/best-python-books/)
`intermediate` `python` `basics`

 - [ ] [How to Work With a PDF in Python](https://realpython.com/pdf-python/)
*Apr 17, 2019* | `intermediate`

 - [ ] Course: [Python Dictionary Iteration: Advanced Tips & Tricks](https://realpython.com/courses/python-dictionary-iteration/)
*Dec 24, 2019* | `intermediate` `python`

 - [ ] Course: [Make a 2D Side-Scroller Game With PyGame](https://realpython.com/courses/pygame-primer/)
*Mar 17, 2020* | `intermediate`

 - [ ] Course: [Emulating switch/case Statements in Python](https://realpython.com/courses/emulating-switch-case-python/)
`intermediate` `python`

 - [ ] [A Beginner’s Guide to the Python time Module](https://realpython.com/python-time-module/)
*Apr 22, 2019* | `intermediate`

 - [ ] [Arcade: A Primer on the Python Game Framework](https://realpython.com/arcade-python-game-framework/)
*Jan 15, 2020* | `intermediate` `python`

 - [ ] [MicroPython: An Intro to Programming Hardware in Python](https://realpython.com/micropython/)
*Dec 09, 2019* | `intermediate` `python`



# [Advanced Python Tutorials](https://realpython.com/tutorials/advanced/)

 - [ ] [Speed Up Your Python Program With Concurrency](https://realpython.com/python-concurrency/)
`best-practices` `advanced`

 - [ ] Course: [Hands-on Python 3 Concurrency With the asyncio Module](https://realpython.com/courses/python-3-concurrency-asyncio-module/)
*Apr 16, 2019* | `advanced` `python`


 - [ ] [Unicode & Character Encodings in Python: A Painless Guide](https://realpython.com/python-encodings-guide/)
*May 20, 2019* | `advanced` `python`

 - [ ] [What is the Python Global Interpreter Lock (GIL)?](https://realpython.com/python-gil/)
`advanced` `python`

 - [ ] [Python Metaclasses](https://realpython.com/python-metaclasses/)
`advanced` `python`

 - [ ] [Your Guide to the CPython Source Code](https://realpython.com/cpython-source-code-guide/)
*Aug 21, 2019* | `advanced` `python`

 - [ ] [Building a Python C Extension Module](https://realpython.com/build-python-c-extension-module/)
*Oct 07, 2019* | `advanced` `python`

 - [ ] Course: [Migrating Applications From Python 2 to Python 3](https://realpython.com/courses/migrating-applications-python-2-python-3/)
`advanced` `python`

 - [ ] Course: [Functional Programming in Python](https://realpython.com/courses/functional-programming-python/)
*Jul 02, 2019* | `advanced` `python`

 - [ ] [Implementing an Interface in Python](https://realpython.com/python-interface/)
*Feb 10, 2020* | `advanced` `python`

 - [ ] [Shallow vs Deep Copying of Python Objects](https://realpython.com/copying-python-objects/)
`advanced` `python`

 - [ ] [Itertools in Python 3, By Example](https://realpython.com/python-itertools/)
`advanced` `python`

 - [ ] [Alexa Python Development: Build and Deploy an Alexa Skill](https://realpython.com/alexa-python-skill/)
*Mar 04, 2020* | `advanced` `python`




# [Python Best Practices for More Pythonic Code](https://realpython.com/tutorials/best-practices/)

 - [ ] Course: [Python Coding Interviews: Tips & Best Practices](https://realpython.com/courses/python-coding-interviews-tips-best-practices/)
*Apr 21, 2020* | `best-practices`

 - [ ] Course: [Inheritance and Composition: A Python OOP Guide](https://realpython.com/courses/inheritance-composition-python/)
*Apr 14, 2020* | `python` `best-practices` `intermediate`

 - [ ] [Writing Comments in Python (Guide)](https://realpython.com/python-comments-guide/)
`best-practices` `python` `basics`

 - [ ] Course: [Writing Comments in Python](https://realpython.com/courses/writing-comments-python/)
`best-practices` `python` `basics`

 - [ ] [How to Write Beautiful Python Code With PEP 8](https://realpython.com/python-pep8/)
`best-practices` `intermediate` `python`

 - [ ] Course: [Writing Beautiful Pythonic Code With PEP 8](https://realpython.com/courses/writing-beautiful-python-code-pep-8/)
`best-practices` `intermediate` `python`

 - [ ] Course: [How to Write Pythonic Loops](https://realpython.com/courses/how-to-write-pythonic-loops/)
*Jul 16, 2019* | `best-practices` `python` `basics`

 - [ ] Course: [Using List Comprehensions Effectively](https://realpython.com/courses/using-list-comprehensions-effectively/)
`intermediate` `python`

 - [ ] [How to Use Python Lambda Functions](https://realpython.com/python-lambda/)
*Jun 19, 2019* | `best-practices` `intermediate` `python`

 - [ ] Course: [How to Use Python Lambda Functions](https://realpython.com/courses/python-lambda-functions/)
*Aug 27, 2019* | `best-practices` `intermediate` `python`

 - [ ] Course: [Idiomatic Python 101](https://realpython.com/courses/idiomatic-python-101/)
`best-practices` `python` `basics`

 - [ ] [Python String Formatting Best Practices](https://realpython.com/python-string-formatting/)
`best-practices` `python` `basics`

 - [ ] Course: [Python String Formatting Tips & Best Practices](https://realpython.com/courses/python-string-formatting-tips-best-practices/)
*Apr 11, 2019* | `best-practices` `python` `basics`

 - [ ] [Documenting Python Code: A Complete Guide](https://realpython.com/documenting-python-code/)
`best-practices` `intermediate` `python`

 - [ ] Course: [Documenting Python Code: A Complete Guide](https://realpython.com/courses/documenting-python-code/)
*Dec 17, 2019* | `best-practices` `intermediate` `python`

 - [ ] Course: [Documenting Python Projects With Sphinx and Read The Docs](https://realpython.com/courses/documenting-python-projects-sphinx-read-the-docs/)
`best-practices` `intermediate`

 - [ ] [Refactoring Python Applications for Simplicity](https://realpython.com/python-refactoring/)
`best-practices` `intermediate`

 - [ ] [Python Application Layouts: A Reference](https://realpython.com/python-application-layouts/)
`best-practices` `python` `basics`

 - [ ] [How to Stand Out in a Python Coding Interview](https://realpython.com/python-coding-interview-tips/)
`best-practices`

 - [ ] [Python Code Quality: Tools & Best Practices](https://realpython.com/python-code-quality/)
`best-practices` `tools` `python`

 - [ ] Course: [Writing Cleaner Python Code With PyLint](https://realpython.com/courses/writing-cleaner-python-code-pylint/)
*May 14, 2019* | `intermediate` `python`

 - [ ] [Python Type Checking (Guide)](https://realpython.com/python-type-checking/)
`best-practices` `intermediate`

 - [ ] Course: [Python Type Checking](https://realpython.com/courses/python-type-checking/)
*Oct 29, 2019* | `best-practices` `intermediate`




# [Python Testing Tutorials](https://realpython.com/tutorials/testing/)

 - [ ] [Getting Started With Testing in Python](https://realpython.com/python-testing/)
`best-practices` `intermediate` `testing`

 - [ ] Course: [Test-Driven Development With PyTest](https://realpython.com/courses/test-driven-development-pytest/)
`testing` `intermediate` `python`

 - [ ] [4 Techniques for Testing Python Command-Line (CLI) Apps](https://realpython.com/python-cli-testing/)
`best-practices` `testing`

 - [ ] [Understanding the Python Mock Object Library](https://realpython.com/python-mock-library/)
`intermediate` `testing`

 - [ ] [Mocking External APIs in Python](https://realpython.com/testing-third-party-apis-with-mocks/)
`advanced` `web-dev` `api` `testing`

 - [ ] [Testing External APIs With Mock Servers](https://realpython.com/testing-third-party-apis-with-mock-servers/)
`advanced` `web-dev` `api` `testing`

 - [ ] [Effective Python Testing With Pytest](https://realpython.com/pytest-python-testing/)
`testing` `intermediate` `python`

 - [ ] [Modern Web Automation With Python and Selenium](https://realpython.com/modern-web-automation-with-python-and-selenium/)
`intermediate` `web-scraping` `testing`

 - [ ] [Headless Selenium Testing with Python and PhantomJS](https://realpython.com/headless-selenium-testing-with-python-and-phantomjs/)
`intermediate` `front-end` `testing` `web-dev` `web-scraping`




# [Python Development Tools Tutorials](https://realpython.com/tutorials/tools/)

 - [ ] [Python IDEs and Code Editors (Guide)](https://realpython.com/python-ides-code-editors-guide/)
`tools` `basics`

   - [ ] [Thonny: The Beginner-Friendly Python Editor](https://realpython.com/python-thonny/)
`tools` `basics`

   - [ ] Course: [Thonny: The Beginner-Friendly Python Editor](https://realpython.com/courses/python-thonny/)
*Sep 24, 2019* | `tools` `basics`

   - [ ] [Setting Up Sublime Text 3 for Full Stack Python Development](https://realpython.com/setting-up-sublime-text-3-for-full-stack-python-development/)
`tools` `basics`

   - [ ] [Jupyter Notebook: An Introduction](https://realpython.com/jupyter-notebook-introduction/)
`tools` `intermediate`

   - [ ] [PyCharm for Productive Python Development (Guide)](https://realpython.com/pycharm-guide/)
*Aug 28, 2019* | `tools` `intermediate`

   - [ ] [Emacs: The Best Python Editor?](https://realpython.com/emacs-the-best-python-editor/)
*Oct 09, 2019* | `tools` `intermediate`

   - [ ] [VIM and Python – A Match Made in Heaven](https://realpython.com/vim-and-python-a-match-made-in-heaven/)
`tools` `intermediate`

   - [ ] Course: [Using Jupyter Notebooks](https://realpython.com/courses/using-jupyter-notebooks/)
`tools` `intermediate`

   - [ ] [Python Development in Visual Studio Code](https://realpython.com/python-development-visual-studio-code/)
`tools` `intermediate`

   - [ ] Course: [Python Development in Visual Studio Code (Setup Guide)](https://realpython.com/courses/python-development-visual-studio-code-setup-guide/)
`tools` `intermediate`

 - [ ] Course: [Finding the Perfect Python Code Editor](https://realpython.com/courses/finding-perfect-python-code-editor/)
*Feb 18, 2020* | `tools` `basics`

 - [ ] [An Effective Python Environment: Making Yourself at Home](https://realpython.com/effective-python-environment/)
*Aug 14, 2019* | `tools` `basics`

 - [ ] [What Is Pip? A Guide for New Pythonistas](https://realpython.com/what-is-pip/)
`tools` `basics`

 - [ ] [Python Virtual Environments: A Primer](https://realpython.com/python-virtual-environments-a-primer/)
`best-practices` `tools` `intermediate` `python`

 - [ ] Course: [Working With Python Virtual Environments](https://realpython.com/courses/working-python-virtual-environments/)
`tools` `basics`

 - [ ] [Managing Multiple Python Versions With pyenv](https://realpython.com/intro-to-pyenv/)
`tools` `intermediate`

 - [ ] [Pipenv: A Guide to the New Python Packaging Tool](https://realpython.com/pipenv-guide/)
`tools` `intermediate`

 - [ ] [Python Logging: A Stroll Through the Source Code](https://realpython.com/python-logging-source-code/)
*May 22, 2019* | `best-practices` `intermediate`

 - [ ] [Logging in Python](https://realpython.com/python-logging/)
`tools` `intermediate`

 - [ ] Course: [Logging in Python](https://realpython.com/courses/logging-python/)
*Jul 23, 2019* | `tools` `intermediate`

 - [ ] [Python Debugging With Pdb](https://realpython.com/python-debugging-pdb/)
`tools` `intermediate` `python`

 - [ ] Course: [Python Debugging With pdb](https://realpython.com/courses/python-debugging-pdb/)
*Sep 17, 2019* | `tools` `intermediate` `python`

 - [ ] [Using PyInstaller to Easily Distribute Python Applications](https://realpython.com/pyinstaller-python/)
`best-practices` `intermediate`

 - [ ] [How to Publish an Open-Source Python Package to PyPI](https://realpython.com/pypi-publish-python-package/)
`best-practices` `tools` `intermediate`

 - [ ] Course: [How to Publish Your Own Python Package to PyPI](https://realpython.com/courses/how-to-publish-your-own-python-package-pypi/)
*Jun 18, 2019* | `best-practices` `tools` `intermediate`

 - [ ] [SimPy: Simulating Real-World Processes With Python](https://realpython.com/simpy-simulating-with-python/)
*Mar 25, 2020* | `tools` `intermediate`

 - [ ] [Python Bindings: Calling C or C++ From Python](https://realpython.com/python-bindings-overview/)
*Mar 02, 2020* | `advanced` `tools` `python`

 - [ ] [Introduction to Git and GitHub for Python Developers](https://realpython.com/python-git-github-intro/)
`tools` `basics`

 - [ ] Course: [Introduction to Git and GitHub for Python Developers](https://realpython.com/courses/python-git-github-intro/)
*Jun 04, 2019* | `tools` `intermediate`

 - [ ] [Advanced Git Tips for Python Developers](https://realpython.com/advanced-git-for-pythonistas/)
`advanced` `tools`




# [Python Docker Tutorials](https://realpython.com/tutorials/docker/)

 - [ ] [Run Python Versions in Docker: How to Try the Latest Python Release](https://realpython.com/python-versions-docker/)
*Dec 18, 2019* | `intermediate` `docker`

 - [ ] [Docker in Action – Fitter, Happier, More Productive](https://realpython.com/docker-in-action-fitter-happier-more-productive/)
`devops` `flask` `docker` `web-dev`




# [Python DevOps Tutorials](https://realpython.com/tutorials/devops/)

 - [ ] [Python, Boto3, and AWS S3: Demystified](https://realpython.com/python-boto3-aws-s3/)
`devops` `intermediate`

 - [ ] Course: [Python, Boto3, and AWS S3: Demystified](https://realpython.com/courses/python-boto3-aws-s3/)
*Dec 03, 2019* | `devops` `intermediate`

 - [ ] [Simplifying Offline Python Deployments With Docker](https://realpython.com/offline-python-deployments-with-docker/)
`devops` `intermediate` `docker` `python`

 - [ ] [Continuous Integration With Python: An Introduction](https://realpython.com/python-continuous-integration/)
`best-practices` `devops` `intermediate` `testing`

 - [ ] Course: [Continuous Integration With Python](https://realpython.com/courses/python-continuous-integration/)
*May 28, 2019* | `best-practices` `devops` `intermediate` `testing`




# [Python API Tutorials](https://realpython.com/tutorials/api/)

 - [ ] [How to Make a Discord Bot in Python](https://realpython.com/how-to-make-a-discord-bot-python/)
*Aug 19, 2019* | `intermediate` `api`

 - [ ] [How to Make a Twitter Bot in Python With Tweepy](https://realpython.com/twitter-bot-python-tweepy/)
*Jun 10, 2019* | `intermediate` `docker` `api`

 - [ ] [Create a REST API in Minutes With Pyramid and Ramses](https://realpython.com/create-a-rest-api-in-minutes-with-pyramid-and-ramses/)
`api` `web-dev`

 - [ ] [API Integration in Python – Part 1](https://realpython.com/api-integration-in-python/)
`api` `web-dev` `basics`




# [Python Community Articles and Interviews](https://realpython.com/tutorials/community/)

 - [ ] [A Pythonista's Holiday Wish List](https://realpython.com/python-holiday-wish-list/)
`community`

 - [ ] [The Ultimate List of Python YouTube Channels](https://realpython.com/python-youtube-channels/)
`community` `python`

 - [ ] [Python Community Interview With Michael Kennedy](https://realpython.com/interview-michael-kennedy/)
`community`

 - [ ] Course: [Welcome to Real Python!](https://realpython.com/courses/real-python-welcome/)
`community` `basics`

 - [ ] [Python Community Interview With Marlene Mhangami](https://realpython.com/interview-marlene-mhangami/)
*Jun 17, 2019* | `community`

 - [ ] [Real Python at PyCon US 2019](https://realpython.com/real-python-pycon-us/)
*Jul 15, 2019* | `community`

 - [ ] [We're Celebrating 1 Million Page Views per Month!](https://realpython.com/one-million-pageviews-celebration/)
`community`

 - [ ] [Python Community Interview With Brett Slatkin](https://realpython.com/interview-brett-slatkin/)
*Feb 12, 2020* | `community`

 - [ ] [Top 10 Must-Watch PyCon Talks](https://realpython.com/must-watch-pycon-talks/)
`community`

 - [ ] [Python Community Interview With Emily Morehouse](https://realpython.com/interview-emily-morehouse/)
`community`

 - [ ] [Python Community Interview With Al Sweigart](https://realpython.com/interview-al-sweigart/)
*Oct 28, 2019* | `community`

 - [ ] [Python Community Interview With Mahdi Yusuf](https://realpython.com/interview-mahdi-yusuf/)
`community`

 - [ ] [The Real Python Podcast Is Here!](https://realpython.com/real-python-podcast-launch/)
*Mar 20, 2020* | `community`

 - [ ] [Python Community Interview With Ali Spittel](https://realpython.com/interview-ali-spittel/)
`community`

 - [ ] [Python Community Interview With Kenneth Reitz](https://realpython.com/interview-kenneth-reitz/)
`community`

 - [ ] [Python Community Interview With Brian Peterson](https://realpython.com/interview-brian-peterson/)
`community`

 - [ ] [Python Community Interview With Mariatta Wijaya](https://realpython.com/interview-mariatta-wijaya/)
`community`

 - [ ] [Python Community Interview With Mike Grouchy](https://realpython.com/interview-mike-grouchy/)
`community`

 - [ ] [How to Get the Most Out of PyCon](https://realpython.com/pycon-guide/)
*Apr 29, 2019* | `community`

 - [ ] [Python Community Interview With Bob and Julian of PyBites](https://realpython.com/python-community-interview-bob-and-julian-pybites/)
*May 08, 2019* | `community`

 - [ ] [PyCon Africa 2019 (Recap)](https://realpython.com/pycon-africa-2019-recap/)
*Nov 11, 2019* | `community`

 - [ ] [Python Community Interview With Brian Okken](https://realpython.com/interview-brian-okken/)
*Nov 25, 2019* | `community`

 - [ ] [Python Community Interview With Corey Schafer](https://realpython.com/interview-corey-schafer/)
`community`

 - [ ] [Python Community Interview With Kelly and Sean of Teaching Python](https://realpython.com/python-community-interview-kelly-and-sean-teaching-python/)
*Jan 27, 2020* | `community`

 - [ ] [Python Community Interview With Katrina Durance](https://realpython.com/interview-katrina-durance/)
*Jun 26, 2019* | `community`

 - [ ] [Python Community Interview With Mike Driscoll](https://realpython.com/interview-mike-driscoll/)
`community`




# [Python Database Tutorials](https://realpython.com/tutorials/databases/)

 - [ ] [Introduction to MongoDB and Python](https://realpython.com/introduction-to-mongodb-and-python/)
`databases` `basics`

 - [ ] [Introduction to Python SQL Libraries](https://realpython.com/python-sql-libraries/)
*Feb 24, 2020* | `databases` `tools` `basics`

 - [ ] [Data Engineer Interview Questions With Python](https://realpython.com/data-engineer-interview-questions-python/)
*Dec 11, 2019* | `databases` `intermediate` `devops`

 - [ ] [How to Use Redis With Python](https://realpython.com/python-redis/)
*Jul 01, 2019* | `databases` `intermediate`

 - [ ] [Preventing SQL Injection Attacks With Python](https://realpython.com/prevent-python-sql-injection/)
*Sep 30, 2019* | `best-practices` `databases` `intermediate`



# [Python Web Development Tutorials](https://realpython.com/tutorials/web-dev/)

 - [ ] [Exploring HTTPS With Python](https://realpython.com/python-https/)
*Jan 08, 2020* | `intermediate` `web-dev`

 - [ ] [Code Evaluation With AWS Lambda and API Gateway](https://realpython.com/code-evaluation-with-aws-lambda-and-api-gateway/)
`advanced` `devops` `web-dev`

 - [ ] Course: [Sending Emails With Python](https://realpython.com/courses/sending-emails-python/)
*Apr 23, 2019* | `intermediate` `web-dev`

 - [ ] [Building a Simple Web App With Bottle, SQLAlchemy, and the Twitter API](https://realpython.com/building-a-simple-web-app-with-bottle-sqlalchemy-twitter-api/)
`databases` `api` `web-dev`

 - [ ] [13 Project Ideas for Intermediate Python Developers](https://realpython.com/intermediate-python-project-ideas/)
`intermediate` `web-dev`

 - [ ] [Sending Emails With Python](https://realpython.com/python-send-email/)
`intermediate` `web-dev`

 - [ ] [Python-driven Web Applications](https://realpython.com/python-web-applications/)
`web-dev` `basics`

 - [ ] [Automatically Scale Heroku Dynos](https://realpython.com/automatically-scale-heroku-dynos/)
`advanced` `devops` `web-dev`

 - [ ] [Building Serverless Python Apps Using AWS Chalice](https://realpython.com/aws-chalice-serverless-python/)
`intermediate` `web-dev`

 - [ ] [Model-View-Controller (MVC) Explained – With Legos](https://realpython.com/the-model-view-controller-mvc-paradigm-summarized-with-legos/)
`intermediate` `web-dev`

 - [ ] [Python + Memcached: Efficient Caching in Distributed Applications](https://realpython.com/python-memcache-efficient-caching/)
`databases` `intermediate` `web-dev`

 - [ ] [Web2py – Migrating from SQLite to MySQL](https://realpython.com/web2py-migrating-from-sqlite-to-mysql/)
`advanced` `databases` `web-dev`

 - [ ] [Python Project for Beginners: Bitcoin Price Notifications](https://realpython.com/python-bitcoin-ifttt/)
`web-dev` `basics`

 - [ ] [Socket Programming in Python (Guide)](https://realpython.com/python-sockets/)
`advanced` `web-dev` `python`

 - [ ] [Developing with Bottle](https://realpython.com/developing-with-bottle-part-1/)
`web-dev` `basics`

 - [ ] [Developing with Bottle – Part 2 (plot.ly API)](https://realpython.com/developing-with-bottle-part-2-plot-ly-api/)
`data-science` `web-dev`



# [Front-end Web Development Tutorials](https://realpython.com/tutorials/front-end/)

 - [ ] [An Effective Sales Page with Bootstrap 3](https://realpython.com/an-effective-sales-page-with-bootstrap-3/)
`front-end` `web-dev` `basics`

 - [ ] [Build a Mobile Application With the Kivy Python Framework](https://realpython.com/mobile-app-kivy-python/)
*Nov 04, 2019* | `intermediate` `front-end`

 - [ ] [Getting Started with Bootstrap 3](https://realpython.com/getting-started-with-bootstrap-3/)
`front-end` `web-dev` `basics`



# [Python Web Scraping Tutorials](https://realpython.com/tutorials/web-scraping/)

 - [ ] [Python’s Requests Library (Guide)](https://realpython.com/python-requests/)
`intermediate` `web-dev`

 - [ ] Course: [Making HTTP Requests With Python](https://realpython.com/courses/python-requests/)
`intermediate` `web-dev`

 - [ ] [Practical Introduction to Web Scraping in Python](https://realpython.com/python-web-scraping-practical-introduction/)
`web-scraping` `basics`

 - [ ] [Beautiful Soup: Build a Web Scraper With Python](https://realpython.com/beautiful-soup-web-scraper-python/)
*Dec 04, 2019* | `tools` `data-science` `intermediate` `web-scraping`

 - [ ] [Web Scraping with Scrapy and MongoDB](https://realpython.com/web-scraping-with-scrapy-and-mongodb/)
`databases` `web-scraping`

 - [ ] [Web Scraping and Crawling with Scrapy and MongoDB](https://realpython.com/web-scraping-and-crawling-with-scrapy-and-mongodb/)
`databases` `web-scraping`




# [Flask Tutorials](https://realpython.com/tutorials/flask/)

 - [ ] [Python Web Applications with Flask – Part I](https://realpython.com/python-web-applications-with-flask-part-i/)
`flask` `web-dev` `basics`

 - [ ] [Python Web Applications With Flask – Part II](https://realpython.com/python-web-applications-with-flask-part-ii/)
`flask` `web-dev`

 - [ ] [Python Web Applications With Flask – Part III](https://realpython.com/python-web-applications-with-flask-part-iii/)
`flask` `web-dev`

 - [ ] [Discover Flask, Part 1 – Setting Up a Static Site](https://realpython.com/introduction-to-flask-part-1-setting-up-a-static-site/)
`flask` `web-dev` `basics`

 - [ ] [Discover Flask, Part 2 – Creating a Login Page](https://realpython.com/introduction-to-flask-part-2-creating-a-login-page/)
`flask` `web-dev` `basics`

 - [ ] [Flask by Example – Project Setup](https://realpython.com/flask-by-example-part-1-project-setup/)
`devops` `flask` `web-dev` `basics`

 - [ ] [Flask by Example – Updating the Staging Environment](https://realpython.com/updating-the-staging-environment/)
`devops` `flask` `web-dev`

 - [ ] [Flask by Example – Updating the UI](https://realpython.com/flask-by-example-updating-the-ui/)
`front-end` `flask` `web-dev` `basics`

 - [ ] [Flask by Example – Setting up Postgres, SQLAlchemy, and Alembic](https://realpython.com/flask-by-example-part-2-postgres-sqlalchemy-and-alembic/)
`databases` `flask` `devops` `web-dev`

 - [ ] [Flask by Example – Implementing a Redis Task Queue](https://realpython.com/flask-by-example-implementing-a-redis-task-queue/)
`databases` `flask` `web-dev`

 - [ ] [Flask by Example – Text Processing with Requests, BeautifulSoup, and NLTK](https://realpython.com/flask-by-example-part-3-text-processing-with-requests-beautifulsoup-nltk/)
`data-science` `flask` `web-dev`

 - [ ] [Flask by Example – Integrating Flask and Angular](https://realpython.com/flask-by-example-integrating-flask-and-angularjs/)
`flask` `web-dev` `front-end`

 - [ ] [Flask by Example – Custom Angular Directive with D3](https://realpython.com/flask-by-example-custom-angular-directive-with-d3/)
`flask` `web-dev` `front-end`

 - [ ] [Python REST APIs With Flask, Connexion, and SQLAlchemy](https://realpython.com/flask-connexion-rest-api/)
`intermediate` `flask` `api` `web-dev`

 - [ ] [Python REST APIs With Flask, Connexion, and SQLAlchemy – Part 2](https://realpython.com/flask-connexion-rest-api-part-2/)
`intermediate` `databases` `web-dev` `flask` `api`

 - [ ] [Python REST APIs With Flask, Connexion, and SQLAlchemy – Part 3](https://realpython.com/flask-connexion-rest-api-part-3/)
`intermediate` `databases` `web-dev` `flask` `api`

 - [ ] [Python REST APIs With Flask, Connexion, and SQLAlchemy – Part 4](https://realpython.com/flask-connexion-rest-api-part-4/)
*Oct 30, 2019* | `intermediate` `flask` `web-dev` `front-end`

 - [ ] [The Minimum Viable Test Suite](https://realpython.com/the-minimum-viable-test-suite/)
`intermediate` `flask` `testing` `web-dev`

 - [ ] [The Ultimate Flask Front-End](https://realpython.com/the-ultimate-flask-front-end/)
`advanced` `flask` `web-dev` `front-end`

 - [ ] [The Ultimate Flask Front-End – Part 2](https://realpython.com/the-ultimate-flask-front-end-part-2/)
`advanced` `flask` `web-dev` `front-end`

 - [ ] [Handling Email Confirmation During Registration in Flask](https://realpython.com/handling-email-confirmation-in-flask/)
`databases` `intermediate` `flask` `web-dev`

 - [ ] [Create a Flask Application With Google Login](https://realpython.com/flask-google-login/)
*Jul 17, 2019* | `intermediate` `flask` `python` `web-dev`

 - [ ] [Dockerizing Flask With Compose and Machine – From Localhost to the Cloud](https://realpython.com/dockerizing-flask-with-compose-and-machine-from-localhost-to-the-cloud/)
`advanced` `web-dev` `devops` `flask` `docker`

 - [ ] [Scaffold a Flask Project](https://realpython.com/scaffold-a-flask-project/)
`flask` `web-dev` `basics`

 - [ ] [Kickstarting Flask on Ubuntu – Setup and Deployment](https://realpython.com/kickstarting-flask-on-ubuntu-setup-and-deployment/)
`devops` `flask` `web-dev`

 - [ ] [Getting Started With the Slack API Using Python and Flask](https://realpython.com/getting-started-with-the-slack-api-using-python-and-flask/)
`intermediate` `flask` `api` `web-dev`

 - [ ] [User Authentication With Angular 4 and Flask](https://realpython.com/user-authentication-with-angular-4-and-flask/)
`advanced` `flask` `web-dev` `front-end`

 - [ ] [Rethink Flask – A Simple Todo List Powered by Flask and RethinkDB](https://realpython.com/rethink-flask-a-simple-todo-list-powered-by-flask-and-rethinkdb/)
`databases` `flask` `web-dev`

 - [ ] [Handling User Authentication With Angular and Flask](https://realpython.com/handling-user-authentication-with-angular-and-flask/)
`flask` `web-dev` `front-end`

 - [ ] [Using Flask-Login for User Management with Flask](https://realpython.com/using-flask-login-for-user-management-with-flask/)
`flask` `web-dev`

 - [ ] [Python, Ruby, and Golang: A Web Service Application Comparison](https://realpython.com/python-ruby-and-golang-a-web-service-application-comparison/)
`databases` `flask` `web-dev`

 - [ ] [Caching External API Requests](https://realpython.com/caching-external-api-requests/)
`advanced` `flask` `api` `web-dev`

 - [ ] [Use a Flask Blueprint to Architect Your Applications](https://realpython.com/flask-blueprint/)
*Feb 03, 2020* | `intermediate` `flask` `web-dev`

 - [ ] [Token-Based Authentication With Flask](https://realpython.com/token-based-authentication-with-flask/)
`advanced` `flask` `web-dev`

 - [ ] [Lyricize: A Flask app to create lyrics using Markov chains](https://realpython.com/lyricize-a-flask-app-to-create-lyrics-using-markov-chains/)
`flask` `web-dev`

 - [ ] [Primer on Jinja Templating](https://realpython.com/primer-on-jinja-templating/)
`flask` `web-dev` `basics`




# [Django Tutorials](https://realpython.com/tutorials/django/)

 - [ ] [Starting a Django Project](https://realpython.com/django-setup/)
`best-practices` `django` `web-dev` `basics`

 - [ ] [Get Started With Django Part 1: Build a Portfolio App](https://realpython.com/get-started-with-django-1/)
`intermediate` `django` `web-dev`

 - [ ] Course: [Get Started With Django: Build a Portfolio App](https://realpython.com/courses/django-portfolio-project/)
*Oct 08, 2019* | `intermediate` `django` `web-dev`

 - [ ] [Getting Started with Django Channels](https://realpython.com/getting-started-with-django-channels/)
`advanced` `django` `databases` `web-dev`

 - [ ] [Django Migrations: A Primer](https://realpython.com/django-migrations-a-primer/)
`databases` `django` `web-dev` `basics`

 - [ ] [Data Migrations](https://realpython.com/data-migrations/)
`databases` `django` `web-dev`

 - [ ] Course: [Django Migrations 101](https://realpython.com/courses/django-migrations-101/)
`intermediate` `web-dev`

 - [ ] [Digging Deeper Into Django Migrations](https://realpython.com/digging-deeper-into-migrations/)
*Jul 24, 2019* | `advanced` `django` `databases` `web-dev`

 - [ ] Course: [Make a Location-Based Web App With Django and GeoDjango](https://realpython.com/courses/make-location-based-web-app-django-and-geodjango/)
*Apr 30, 2019* | `intermediate` `django` `web-dev`

 - [ ] [The Ultimate Guide to Django Redirects](https://realpython.com/django-redirects/)
`intermediate` `django` `web-dev`

 - [ ] [Adding Social Authentication to Django](https://realpython.com/adding-social-authentication-to-django/)
`django` `web-dev`

 - [ ] [Caching in Django With Redis](https://realpython.com/caching-in-django-with-redis/)
`databases` `intermediate` `django` `web-dev`

 - [ ] [How to Provide Test Fixtures for Django Models in Pytest](https://realpython.com/django-pytest-fixtures/)
*Apr 08, 2020* | `intermediate` `django` `testing`

 - [ ] [Kickstarting a Django Open Source Project](https://realpython.com/kickstarting-a-django-open-source-project/)
`django` `web-dev`

 - [ ] [Django Rest Framework – Class Based Views](https://realpython.com/django-rest-framework-class-based-views/)
`django` `api` `web-dev`

 - [ ] [Asynchronous Tasks With Django and Celery](https://realpython.com/asynchronous-tasks-with-django-and-celery/)
`advanced` `django` `databases` `web-dev`

 - [ ] [Fun with Django's New Postgres Features](https://realpython.com/fun-with-djangos-new-postgres-features/)
`advanced` `django` `databases` `web-dev`

 - [ ] [LinkedIn Social Authentication in Django](https://realpython.com/linkedin-social-authentication-in-django/)
`advanced` `django` `web-dev`

 - [ ] [How to Create an Index in Django Without Downtime](https://realpython.com/create-django-index-without-downtime/)
*Apr 10, 2019* | `databases` `intermediate` `django`

 - [ ] [Django and AJAX Form Submissions – Say 'Goodbye' to the Page Refresh](https://realpython.com/django-and-ajax-form-submissions/)
`django` `web-dev` `front-end`

 - [ ] [Create a Super Basic REST API with Django Tastypie](https://realpython.com/create-a-super-basic-rest-api-with-django-tastypie/)
`django` `api` `web-dev`

 - [ ] [Modeling Polymorphism in Django With Python](https://realpython.com/modeling-polymorphism-django-python/)
`intermediate` `django` `web-dev`

 - [ ] [What You Need to Know to Manage Users in Django Admin](https://realpython.com/manage-users-in-django-admin/)
*Aug 05, 2019* | `intermediate` `django` `web-dev`

 - [ ] [Django Rest Framework – An Introduction](https://realpython.com/django-rest-framework-quick-start/)
`django` `api` `web-dev`

 - [ ] [Generating Code Documentation with Pycco](https://realpython.com/generating-code-documentation-with-pycco/)
`advanced` `tools` `django` `web-dev`

 - [ ] [Make a Location-Based Web App With Django and GeoDjango](https://realpython.com/location-based-app-with-geodjango-tutorial/)
`intermediate` `django` `web-dev`

 - [ ] [Django and AJAX Form Submissions – More Practice](https://realpython.com/django-and-ajax-form-submissions-more-practice/)
`django` `web-dev` `front-end`

 - [ ] [Working with Django and Flask on Nitrous.IO](https://realpython.com/working-with-django-and-flask-on-nitrous-io/)
`tools` `flask` `django` `web-dev`


## Django Development

 - [ ] [Automating Django Deployments with Fabric and Ansible](https://realpython.com/automating-django-deployments-with-fabric-and-ansible/)
`advanced` `devops` `web-dev` `django`

 - [ ] [Django Development with Docker Compose and Machine](https://realpython.com/django-development-with-docker-compose-and-machine/)
`django` `advanced` `web-dev` `devops` `docker`

 - [ ] [Deploying Django on Dokku](https://realpython.com/deploying-a-django-app-on-dokku/)
`django` `advanced` `web-dev` `devops` `docker`

 - [ ] [Development and Deployment of Cookiecutter-Django via Docker](https://realpython.com/development-and-deployment-of-cookiecutter-django-via-docker/)
`devops` `docker` `django` `web-dev`

 - [ ] [Development and Deployment of Django on Fedora](https://realpython.com/development-and-deployment-of-cookiecutter-django-on-fedora/)
`devops` `tools` `django` `web-dev`

 - [ ] [Deploying a Django App to AWS Elastic Beanstalk](https://realpython.com/deploying-a-django-app-to-aws-elastic-beanstalk/)
`django` `advanced` `databases` `web-dev` `devops`

 - [ ] [Deploying Django + Python 3 + PostgreSQL to AWS Elastic Beanstalk](https://realpython.com/deploying-a-django-app-and-postgresql-to-aws-elastic-beanstalk/)
`django` `advanced` `databases` `web-dev` `devops`

 - [ ] [Transaction Management with Django 1.6](https://realpython.com/transaction-management-with-django-1-6/)
`advanced` `django` `databases` `web-dev`

 - [ ] [Migrating your Django Project to Heroku](https://realpython.com/migrating-your-django-project-to-heroku/)
`django` `advanced` `databases` `web-dev` `devops`


## Django Testing Tutorials

 - [ ] [Test Driven Development of a Django RESTful API](https://realpython.com/test-driven-development-of-a-django-restful-api/)
`django` `advanced` `testing` `web-dev` `api`

 - [ ] [Django 1.6 Test Driven Development](https://realpython.com/django-1-6-test-driven-development/)
`web-dev` `django` `testing` `basics`

 - [ ] [Integration Testing with pyVows and Django](https://realpython.com/integration-testing-with-pyvows-and-django/)
`web-dev` `django` `testing`

 - [ ] [Unit Testing with pyVows and Django](https://realpython.com/unit-testing-with-pyvows-and-django/)
`web-dev` `django` `testing`

 - [ ] [Asynchronous Testing with Django and pyVows](https://realpython.com/asynchronous-testing-with-django-and-pyvows/)
`web-dev` `django` `testing`

 - [ ] [Testing in Django (Part 1) – Best Practices and Examples](https://realpython.com/testing-in-django-part-1-best-practices-and-examples/)
`django` `testing` `basics` `web-dev` `best-practices`

 - [ ] [Testing in Django (Part 2) – Model Mommy vs Django Testing Fixtures](https://realpython.com/testing-in-django-part-2-model-mommy-vs-django-testing-fixtures/)
`advanced` `web-dev` `django` `testing`



# [Python Data Science Tutorials](https://realpython.com/tutorials/data-science/)

 - [ ] [The Pandas DataFrame: Make Working With Data Delightful](https://realpython.com/pandas-dataframe/)
`data-science` `intermediate`

 - [ ] Course: [Pandas DataFrames 101](https://realpython.com/courses/pandas-dataframes-101/)
`data-science` `basics`

 - [ ] [Combining Data in Pandas With merge(), .join(), and concat()](https://realpython.com/pandas-merge-join-and-concat/)
`data-science` `intermediate`

 - [ ] [Using Pandas and Python to Explore Your Dataset](https://realpython.com/pandas-python-explore-dataset/)
*Jan 06, 2020* | `data-science` `basics`

 - [ ] [Pandas GroupBy: Your Guide to Grouping Data in Python](https://realpython.com/pandas-groupby/)
*Nov 18, 2019* | `data-science` `intermediate` `python`

 - [ ] [Fast, Flexible, Easy and Intuitive: How to Speed Up Your Pandas Projects](https://realpython.com/fast-flexible-pandas/)
`data-science` `python`

 - [ ] [Python Pandas: Tricks & Features You May Not Know](https://realpython.com/python-pandas-tricks/)
`data-science` `intermediate`

 - [ ] [Pandas: How to Read and Write Files](https://realpython.com/pandas-read-write-files/)
*Dec 02, 2019* | `databases` `data-science` `intermediate`

 - [ ] [Using Pandas to Read Large Excel Files in Python](https://realpython.com/working-with-large-excel-files-in-pandas/)
`data-science`

 - [ ] Course: [Idiomatic Pandas: Tricks & Features You May Not Know](https://realpython.com/courses/idiomatic-pandas-tricks-features-you-may-not-know/)
`data-science` `intermediate`

 - [ ] Course: [Introduction to Pandas and Vincent](https://realpython.com/courses/introduction-pandas-and-vincent/)
`data-science` `basics`

 - [ ] [Python Plotting With Matplotlib (Guide)](https://realpython.com/python-matplotlib-guide/)
`data-science` `basics`

 - [ ] [Python Interview Problem – Parsing CSV Files](https://realpython.com/python-interview-problem-parsing-csv-files/)
`data-science` `python` `basics`

 - [ ] [Reading and Writing CSV Files in Python](https://realpython.com/python-csv/)
`data-science` `intermediate` `python`

 - [ ] Course: [Reading and Writing CSV Files](https://realpython.com/courses/reading-and-writing-csv-files/)
`data-science` `intermediate` `python`

 - [ ] [Scientific Python: Using SciPy for Optimization](https://realpython.com/python-scipy-cluster-optimize/)
*Jan 20, 2020* | `data-science` `intermediate`

 - [ ] [NumPy, SciPy, and Pandas: Correlation With Python](https://realpython.com/numpy-scipy-pandas-correlation-python/)
*Dec 23, 2019* | `data-science` `intermediate`

 - [ ] [Python for Social Scientists](https://realpython.com/python-for-social-scientists/)
`data-science` `python`

 - [ ] [Python Statistics Fundamentals: How to Describe Your Data](https://realpython.com/python-statistics/)
*Dec 16, 2019* | `data-science` `intermediate`

 - [ ] [The Ultimate List of Data Science Podcasts](https://realpython.com/data-science-podcasts/)
`community` `data-science`

 - [ ] [NumPy arange(): How to Use np.arange()](https://realpython.com/how-to-use-numpy-arange/)
*Jul 22, 2019* | `data-science` `intermediate`

 - [ ] [Generating Random Data in Python (Guide)](https://realpython.com/python-random/)
`data-science` `intermediate` `python`

 - [ ] Course: [Python Plotting With Matplotlib](https://realpython.com/courses/python-plotting-matplotlib/)
*Oct 22, 2019* | `data-science` `basics`

 - [ ] [Twitter Sentiment Analysis – Python, Docker, Elasticsearch, Kibana](https://realpython.com/twitter-sentiment-python-docker-elasticsearch-kibana/)
`advanced` `web-dev` `data-science` `docker` `api`

 - [ ] [Interactive Data Visualization in Python With Bokeh](https://realpython.com/python-data-visualization-bokeh/)
`data-science` `intermediate`

 - [ ] Course: [Interactive Data Visualization in Python With Bokeh](https://realpython.com/courses/interactive-data-visualization-python-bokeh/)
*May 21, 2019* | `data-science` `intermediate`

 - [ ] [Full Stack Development – Fetching Data, Visualizing with D3, and Deploying with Dokku](https://realpython.com/web-development-with-flask-fetching-data-with-requests/)
`front-end` `web-dev` `devops` `data-science` `flask`

 - [ ] [MATLAB vs Python: Why and How to Make the Switch](https://realpython.com/matlab-vs-python/)
*Sep 09, 2019* | `best-practices` `data-science` `basics`

 - [ ] [First Steps With PySpark and Big Data Processing](https://realpython.com/pyspark-intro/)
*Jul 31, 2019* | `data-science` `intermediate`

 - [ ] [Natural Language Processing With spaCy in Python](https://realpython.com/natural-language-processing-spacy-python/)
*Sep 02, 2019* | `data-science` `intermediate`

 - [ ] [Python Histogram Plotting: NumPy, Matplotlib, Pandas & Seaborn](https://realpython.com/python-histograms/)
`data-science` `basics`

 - [ ] Course: [Python Histogram Plotting: NumPy, Matplotlib, Pandas & Seaborn](https://realpython.com/courses/python-histograms/)
*Aug 20, 2019* | `data-science` `basics`

 - [ ] [Analyzing Obesity in England With Python](https://realpython.com/analyzing-obesity-in-england-with-python/)
`data-science` `basics`

 - [ ] [Pythonic Data Cleaning With Pandas and NumPy](https://realpython.com/python-data-cleaning-numpy-pandas/)
`data-science` `intermediate`

 - [ ] Course: [Using NumPy's np.arange() Effectively](https://realpython.com/courses/numpy-arange/)
*Mar 24, 2020* | `data-science` `intermediate`

 - [ ] Course: [Generating Random Data in Python](https://realpython.com/courses/generating-random-data-python/)
*Jun 25, 2019* | `data-science` `intermediate` `python`

 - [ ] [Setting up a Simple OCR Server](https://realpython.com/setting-up-a-simple-ocr-server/)
`intermediate` `web-dev` `data-science` `flask` `api`

 - [ ] [Look Ma, No For-Loops: Array Programming With NumPy](https://realpython.com/numpy-array-programming/)
`data-science` `intermediate`



# [Python Machine Learning Tutorials](https://realpython.com/tutorials/machine-learning/)

 - [ ] [Setting Up Python for Machine Learning on Windows](https://realpython.com/python-windows-machine-learning-setup/)
`data-science` `machine-learning` `basics`

 - [ ] [Traditional Face Detection With Python](https://realpython.com/traditional-face-detection-python/)
`data-science` `intermediate` `machine-learning`

 - [ ] Course: [Traditional Face Detection With Python](https://realpython.com/courses/traditional-face-detection-python/)
*Aug 13, 2019* | `data-science` `intermediate` `machine-learning`

 - [ ] [Face Recognition with Python, in Under 25 Lines of Code](https://realpython.com/face-recognition-with-python/)
`data-science` `machine-learning`

 - [ ] [Face Detection in Python Using a Webcam](https://realpython.com/face-detection-in-python-using-a-webcam/)
`data-science` `machine-learning`

 - [ ] [Fingerprinting Images for Near-Duplicate Detection](https://realpython.com/fingerprinting-images-for-near-duplicate-detection/)
`data-science` `machine-learning`

 - [ ] [Image Segmentation Using Color Spaces in OpenCV + Python](https://realpython.com/python-opencv-color-spaces/)
`intermediate` `machine-learning`

 - [ ] [Linear Regression in Python](https://realpython.com/linear-regression-in-python/)
*Apr 15, 2019* | `data-science` `intermediate` `machine-learning`

 - [ ] [Practical Text Classification With Python and Keras](https://realpython.com/python-keras-text-classification/)
`advanced` `data-science` `machine-learning`

 - [ ] [Logistic Regression in Python](https://realpython.com/logistic-regression-python/)
*Jan 13, 2020* | `data-science` `intermediate` `machine-learning`

 - [ ] [Pure Python vs NumPy vs TensorFlow Performance Comparison](https://realpython.com/numpy-tensorflow-performance/)
`advanced` `data-science` `machine-learning`

 - [ ] [The Ultimate Guide To Speech Recognition With Python](https://realpython.com/python-speech-recognition/)
`advanced` `data-science` `machine-learning`

 - [ ] [Three Ways of Storing and Accessing Lots of Images in Python](https://realpython.com/storing-images-in-python/)
*May 15, 2019* | `intermediate` `machine-learning`

 - [ ] [Build a Recommendation Engine With Collaborative Filtering](https://realpython.com/build-recommendation-engine-collaborative-filtering/)
*Jul 10, 2019* | `data-science` `intermediate` `machine-learning`


